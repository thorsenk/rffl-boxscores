...
def load_playoff_pairs_from_h2h(year: int) -> Dict[Tuple[int, str], Tuple[str, float, float, str]]:
...
def load_playoff_pairs_from_box(year: int) -> Dict[Tuple[int, str], Tuple[str, float, float, str, float, float]]:
    """Infer playoff pairs from boxscores_normalized for weeks >=15 by pairing per (week,matchup)."""
...
def build_playoff_index(years: List[int]) -> Dict[int, Dict[int, Dict[str, Tuple[str, float, float, str]]]]:
    idx: Dict[int, Dict[int, Dict[str, Tuple[str, float, float, str, float, float]]]] = {}
    for y in years:
...
def fill_master_ps(in_path: str, out_path: str) -> Dict[str, int]:
    ...
    counts = {k: 0 for k in [
        "ps_gp","ps_wins","ps_losses","ps_actual_pf","ps_actual_pa",
        "ps_proj_pf","ps_proj_pa",
        "qf_pf","qf_pa","qf_results","qf_opponent_code",
        "sf_pf","sf_pa","sf_results","sf_opponent_code",
        "f_week","f_pf","f_pa","f_result","f_opponent_code",
    ]}
    total = 0
    with open(in_path, newline="", encoding="utf-8") as f_in, open(out_path, "w", newline="", encoding="utf-8") as f_out:
        ...
        for row in r:
            ...
            if any((row.get(k) or "").strip() == "" for k in ("ps_wins","ps_losses","ps_actual_pf","ps_actual_pa","ps_proj_pf","ps_proj_pa")):
                wcnt = lcnt = 0
                pf_sum = pa_sum = 0.0
                ppf_sum = ppa_sum = 0.0
                for wk in weeks:
                    ...
                    if proj_pf is not None and proj_pa is not None:
                        ppf_sum += proj_pf; ppa_sum += proj_pa
                    if res == "W": wcnt += 1
                    elif res == "L": lcnt += 1
                if (row.get("ps_wins") or "").strip() == "":
                    row["ps_wins"] = str(wcnt); counts["ps_wins"] += 1
                if (row.get("ps_losses") or "").strip() == "":
                    row["ps_losses"] = str(lcnt); counts["ps_losses"] += 1
            ...
            if 15 in per_week:
                if (row.get("qf_pf") or "").strip() == "":
                    ...
                    counts["qf_pf"] += 1; counts["qf_pa"] += 1; counts["qf_results"] += 1; counts["qf_opponent_code"] += 1
            # SF (week 16)
            if 16 in per_week and (row.get("sf_pf") or "").strip() == "":
                if code in per_week[16]:
                    ...
                    counts["sf_pf"] += 1; counts["sf_pa"] += 1; counts["sf_results"] += 1; counts["sf_opponent_code"] += 1
            # Final (week 17)
            if 17 in per_week and (row.get("f_pf") or "").strip() == "":
                if code in per_week[17]:
                    tup = per_week[17][code]
                    opp, pf, pa, res = tup[0], tup[1], tup[2], tup[3]
                    row["f_week"] = "17" if (row.get("f_week") or "").strip()=="" else row["f_week"]
                    row["f_pf"] = f"{pf:.2f}".rstrip('0').rstrip('.')
                    row["f_pa"] = f"{pa:.2f}".rstrip('0').rstrip('.')
                    row["f_result"] = res
                    row["f_opponent_code"] = opp
                    counts["f_week"] += 1; counts["f_pf"] += 1; counts["f_pa"] += 1; counts["f_result"] += 1; counts["f_opponent_code"] += 1
            w.writerow(row)
    counts["rows"] = total
    return counts
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fill Postseason fields from normalized h2h/boxscores")
